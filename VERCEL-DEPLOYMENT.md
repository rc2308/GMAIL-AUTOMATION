...
This release replaces spreadsheet name guessing with persistent column mappings, exact source values, and validated AI assistance for unclear layouts. Existing linked spreadsheets begin automatic repair when the signed-in user opens Gather. Jobs continue while the app is open and resume on return, without preview or approval screens. Every import route now refreshes Google Sheets grid dimensions and reads bounded pages across the complete row and column range; the remaining legacy `A1:Z10000` request has been removed. The parser version change automatically rescans already linked spreadsheets. Shared workspaces retain contact IDs, exclusions, notes, phones, and campaign snapshots. API keys and model choices also work for spreadsheet assistance when card extraction is disabled. Templates now accept inline images plus PDF and PowerPoint (`.pptx`) attachments. Large files upload in resumable chunks, the server validates their file signatures, and Gmail receives a standards-compliant MIME message through its media upload endpoint. The combined template files are limited to 25 MB; a Workspace administrator may impose a lower Gmail limit.

Release validation: 107 automated tests, syntax checks, and the production build. A local headless browser verified all 13 screenshot business names, 26 contacts across two shared workspaces, automatic repair, rescan/reload recovery, the correct workspace's edit form, and preserved delivery history. Attachment tests cover resumable PDF/PPTX uploads, restart recovery, type validation, aggregate limits, frozen campaign files, and Gmail MIME output. Provider tests cover Claude, Gemini, and compatible APIs, source validation, transient and permanent failures, missing keys, encrypted credentials, and account isolation. No real emails or AI requests were made during these checks.
...
- Template PDFs and PowerPoint files upload in 2 MB chunks and can be sent with inline images up to a combined 25 MB. Interrupted uploads can resume by job ID; incomplete chunks older than 24 hours are removed. Gmail or a Google Workspace administrator may enforce a lower account-specific limit.
