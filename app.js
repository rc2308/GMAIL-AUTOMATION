...
(() => {
  ...
  function spreadsheetOptions(mode='existing',creating=false){
    ...
    return `<label class="field"><span>Workspace spreadsheet</span><select name="mode">${choices.map(([value,label])=>`<option value="${value}" ${mode===value?'selected':''}>${label}</option>`).join('')}</select></label><div data-sheet-option="existing">${field('Google spreadsheet URL or ID','sheetId','','required placeholder="https://docs.google.com/spreadsheets/d/…" autocomplete="off" spellcheck="false"')}<p class="field-help">Already have your emails and contacts in Google Sheets? Paste its link here. The same spreadsheet can be linked to multiple workspaces; they share its Gather Contacts list. After linking, choose the tab and columns to import.</p></div>${creating?`<div data-sheet-option="create">${field('Spreadsheet name','sheetName','','placeholder="Your workspace name + Contacts"')}</div>`:''}<p class="field-help" data-sheet-option="existing create">${data.connections.sheets.connected?`Use a spreadsheet that <strong>${e(data.connections.sheets.email)}</strong> can edit. Approved contacts sync to a “Gather Contacts” tab; your original tabs stay unchanged.`:'Connect your Google Sheets account before linking or creating a spreadsheet. '+button('Connect Google Sheets','sheet-connections','text')}</p>`;
  }
