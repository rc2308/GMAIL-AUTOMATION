...
(() => {
  ...
  function sheetImportMessage(wid,result,prefix='Spreadsheet connected.'){
    ...
    if(result?.needsContinuation)return `${prefix} Contacts are being imported and repaired automatically.`;
  ...
  function render(){
    ...
    const imports=data.workspaces.filter(w=>w.sheetImport&&!w.sheetImport.finished);
    if(imports.length){const seen=new Set();$('.content').insertAdjacentHTML('afterbegin',imports.filter(w=>!seen.has(w.sheetId)&&seen.add(w.sheetId)).map(w=>{const s=w.sheetImport;return `<div class="notice" role="status"><strong>${e(w.name)}: ${s.status==='waiting'?'Spreadsheet import paused':'Importing and repairing contacts'}</strong> ${s.imported} added · ${s.repaired} repaired · ${s.rowsScanned} rows checked · ${s.pending} awaiting AI · ${s.unresolved} unresolved.${s.error?' '+e(s.error)+(s.phase==='ai'?' <a href="#settings">Open Settings</a>':' <a href="#connections">Check connection</a>'):' Progress is saved automatically; keep Gather open to continue.'}</div>`;}).join(''));}
  ...
  async function run(action,message){
    ...
    if(error)toast(error.message,true);else if(message)toast(typeof message==='function'?message():message);queueMicrotask(startAutomaticWork);return result;
  }
  ...
  async function startAutomaticWork(){
    if(busy||!online||$('#modal')||document.hidden||['settings','templates'].includes(view))return;
    const target=data.workspaces.find(w=>{
      if(!w.sheetId||!data.connections.sheets.connected||w.sheetEmail!==data.connections.sheets.email||(sheetRetryAfter.get(w.id)||0)>Date.now())return false;
      const s=w.sheetImport;
      return !s||s.version!==data.sheetImportVersion||s.status==='running'||s.status==='waiting'&&(!s.retryAt||s.retryAt<=Date.now())&&(!s.error?.includes('API key')||data.settings.hasKey&&data.settings.model);
    });
    if(target){
      sheetRetryAfter.set(target.id,Date.now()+60000);
      const result=await run(async()=>{$('#working').textContent=`Importing and repairing contacts in ${target.name}…`;return api(`/workspaces/${target.id}/import-sheet`,{automatic:true,...(target.sheetImport?.version===data.sheetImportVersion?{jobId:target.sheetImport.jobId}:{})});});
      if(result)sheetRetryAfter.set(target.id,Date.now()+(result.status==='running'?500:60000));
      return;
    }
    return startAutomaticExtraction();
  }
  ...
  function updateDuplicateReview(){
    ...
    const target=scoped('contacts',upload.workspace).find(c=>c.id===reviewTarget),merged=target?C.merge(target,{...candidate,emails:(candidate.emails||[]).filter(C.validEmail)}):null;
  ...
  document.addEventListener('submit',event=>{
    ...
    switch(form.id){
      ...
      case 'settings-form':return run(async()=>{const result=await api('/settings',{...values,enabled:values.enabled==='on',clearKey:values.clearKey==='on'});sheetRetryAfter.clear();return result;},'LLM configuration saved securely on the server.');
  ...
  setInterval(startAutomaticWork,1000);
  ...
  else refresh().then(()=>{render();startAutomaticWork();}).catch(error=>{app.innerHTML='<main class="auth-panel"><p>Unable to load Gather. Check the local server and refresh this page.</p></main>';});
})();
