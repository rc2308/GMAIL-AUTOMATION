...
(() => {
  ...
  function communication(){
    ...
    const recipients=campaigns.flatMap(c=>c.recipients.map(r=>({...r,campaign:c}))),rows=recipients.filter(r=>mailFilter==='all'||r.status===mailFilter);
    const count=status=>recipients.filter(r=>r.status===status).length;
    return title('Communication','Send through Gmail and see exactly which addresses received your message.',button('Send bulk email','compose-all','primary'))+`<div class="sender-bar card"><span class="service-icon gmail">M</span><div><strong>${data.connections.gmail.connected?e(data.connections.gmail.email):'No Gmail account connected'}</strong><small>Each approved address receives a separate email.</small></div><a href="#connections" class="button secondary">Manage connection</a></div><section class="stats">${stat('Campaigns',campaigns.length,'This workspace')}${stat('Sent',count('sent'),'Confirmed by Gmail')}${stat('Pending',count('pending'),'Not submitted to Gmail')}${stat('Needs attention',count('failed')+count('unknown'),'Check delivery details')}</section><div class="campaign-queue">${campaigns.map(c=>{const p=C.campaignProgress(c);return `<article class="card campaign-queue-item"><div><strong>${e(c.name)}</strong><small>${p.sent} / ${p.total} emails sent · ${p.completedBatches} / ${p.totalBatches} batches complete${c.status==='waiting'?` · Waiting until ${date(c.retryAt)}`:''}</small></div>${badge(c.status,c.status==='sent'?'':'gray')}${button(c.status==='draft'?'Review & send':['paused','waiting'].includes(c.status)?'Resume campaign':'View campaign','view-campaign','secondary',`data-id="${c.id}"`)}</article>`;}).join('')}</div><div class="toolbar"><div class="tabs" role="group" aria-label="Filter delivery status">${[['all','All messages'],['sent','Sent'],['pending','Pending'],['failed','Failed'],['unknown','Unknown']].map(([key,label])=>`<button type="button" data-action="mail-filter" data-id="${key}" class="${mailFilter===key?'active':''}">${label}</button>`).join('')}</div></div><div class="card table-wrap"><table class="data-table"><thead><tr><th>Recipient</th><th>Campaign / subject</th><th>Sender</th><th>Status</th><th>Sent at</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td><strong>${e(r.email)}</strong><small>${e(r.business||r.name)}</small></td><td>${e(r.campaign.name)}<small>${e(r.campaign.subject)}</small></td><td>${e(r.campaign.sender)}</td><td>${badge(r.status,r.status==='sent'?'':r.status==='failed'||r.status==='unknown'?'peach':'gray')}${r.error?`<small class="inline-error">${e(r.error)}</small>`:''}</td><td>${date(r.sentAt)}</td><td>${button('View','view-campaign','text',`data-id="${r.campaign.id}"`)}</td></tr>`).join('')||'<tr><td colspan="6" class="table-empty">No messages yet. Create a template, then compose your first campaign.</td></tr>'}</tbody></table></div>`;
  }
  ...
  function chooseWorkspace(wid){selected=wid;localStorage.setItem(workspaceStorageKey(),wid);selectedContacts.clear();excludedEmails.clear();search='';templateDraft=null;render();}
  ...
  function spreadsheetOptions(mode='existing',creating=false){
    ...
    return `<label class="field"><span>Workspace spreadsheet</span><select name="mode">${choices.map(([value,label])=>`<option value="${value}" ${mode===value?'selected':''}>${label}</option>`).join('')}</select></label><div data-sheet-option="existing">${field('Google spreadsheet URL or ID','sheetId','','required placeholder="https://docs.google.com/spreadsheets/d/…" autocomplete="off" spellcheck="false"')}<p class="field-help">Already have your emails and contacts in Google Sheets? Paste its link here. After linking, choose the tab and columns to import.</p></div>${creating?`<div data-sheet-option="create">${field('Spreadsheet name','sheetName','','placeholder="Your workspace name + Contacts"')}</div>`:''}<p class="field-help" data-sheet-option="existing create">${data.connections.sheets.connected?`Use a spreadsheet that <strong>${e(data.connections.sheets.email)}</strong> can edit. Approved contacts sync to a “Gather Contacts” tab; your original tabs stay unchanged.`:'Connect your Google Sheets account before linking or creating a spreadsheet. '+button('Connect Google Sheets','sheet-connections','text')}</p>`;
  }
  ...
  function importSheet(wid=selected,justLinked=false){
    const w=data.workspaces.find(r=>r.id===wid);
    modal(justLinked?'Spreadsheet linked — bring in your contacts':'Import contacts from a tab',`Import into ${e(w.name)}. Match the column headers in your first row; imported contacts are checked in Upload cards before approval.`,`<form id="import-sheet-form" data-workspace="${wid}"><p class="field-help"><a href="https://docs.google.com/spreadsheets/d/${e(w.sheetId)}" target="_blank" rel="noopener">Open ${e(w.sheetName)} ↗</a> to check its tab name and column headers. Your source tab stays unchanged.</p>${field('Source tab name','tab','','required placeholder="e.g. Sheet1"')}<div class="split-fields">${[['name','Contact name'],['business','Business'],['role','Role'],['emails','Email'],['phones','Phone'],['notes','Notes']].map(([key,label])=>field(`${label} column header`,key,label)).join('')}</div><p class="field-help">Map at least an email or phone column. Leave fields blank for columns you do not have. Contacts already in the Gather Contacts tab sync automatically and do not need importing again.</p><div class="modal-actions">${button(justLinked?'Skip for now':'Cancel','close')}<button class="button primary" type="submit">Import for review</button></div></form>`,'',w.name);
  }
  ...
  async function handleAction(action,target){
    ...
    switch(action){
      ...
      case 'import-sheet':return importSheet(target.dataset.id||selected);
  ...
  document.addEventListener('submit',event=>{
    const form=event.target;if(!['password-form','workspace-form','sheet-form','contact-form','review-form','copy-form','import-sheet-form','template-form','settings-form','compose-form'].includes(form.id))return;
    ...
    switch(form.id){
      ...
      case 'workspace-form':return run(async()=>{
        ...
        if(values.mode!=='later')await api(`/workspaces/${targetWorkspace}/sheet`,values);
        chooseWorkspace(targetWorkspace);close();return targetWorkspace;
      },()=>values.mode==='later'?'Workspace created. You can link a spreadsheet from its card.':syncSaveMessage(form.dataset.workspace,'Workspace created and spreadsheet connected.')).then(targetWorkspace=>{if(targetWorkspace&&values.mode==='existing')importSheet(targetWorkspace,true);});
      case 'sheet-form':{const targetWorkspace=form.dataset.workspace;return run(async()=>{$('#working').textContent='Connecting spreadsheet and syncing contacts…';await api(`/workspaces/${targetWorkspace}/sheet`,values);close();return targetWorkspace;},()=>syncSaveMessage(targetWorkspace,'Spreadsheet connected.')).then(linked=>{if(linked&&values.mode==='existing')importSheet(linked,true);});}
      ...
      case 'import-sheet-form':{let count=0;return run(async()=>{const {tab,...columns}=values;const result=await api(`/workspaces/${form.dataset.workspace}/import-sheet`,{tab,columns});count=result.count;chooseWorkspace(form.dataset.workspace);close();view='upload';location.hash='upload';},()=>`${count} spreadsheet row${count===1?'':'s'} imported for review.`);}
