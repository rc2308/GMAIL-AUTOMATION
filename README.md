...
- **Workspaces:** create occasions/projects, each with one connected Google spreadsheet. The same spreadsheet can be reused across multiple workspaces, which gives those workspaces a shared `Gather Contacts` list. Choose **Link existing spreadsheet** on a workspace card, or link a sheet while creating a workspace by pasting its Google Sheets URL/ID. Gather immediately scans every source tab for email addresses and infers nearby business and contact names. Headers may appear below title rows, columns can be in any order, and label/value or unlabelled rows are supported. Valid contacts are saved directly and synced to the dedicated `Gather Contacts` tab; original tabs stay unchanged. Use **Scan for contacts** later to pick up new source rows without creating duplicates.
