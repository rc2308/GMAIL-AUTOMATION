...
Spreadsheet scans use Google's [grid row and column counts](https://developers.google.com/workspace/sheets/api/reference/rest/v4/spreadsheets/sheets#GridProperties), with resumable pages of at most 200 rows or about 20,000 cells. There is no fixed row-10,000 or ZZ-column cutoff. Source data is read before corrections are written to `Gather Contacts`. Names are matched to existing contacts by a unique Gather ID, otherwise a unique normalized email match. Conflicting matches keep their email contacts with unresolved names blank. Clear source columns take precedence over old managed-tab guesses; email domains never supply business names. Existing contact IDs, exclusions, phones, unrelated notes, and frozen campaign recipients are preserved. Corrections synchronize across workspaces linked to that sheet in the same account.
