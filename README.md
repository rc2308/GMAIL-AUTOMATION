...
- **Workspaces:** create occasions/projects, each with one connected Google spreadsheet. The same spreadsheet can be reused across workspaces in your account, sharing a `Gather Contacts` list. Link an existing sheet by its URL/ID to start automatic import and repair. Column mappings survive blank rows and processing batches, and repeated/reordered headers start a new table section. Clearly labelled business and personal names are copied exactly, including empty cells; email-only contacts are retained. Use **Scan for contacts** to read the latest source rows and repair existing imports without a preview or approval screen.
- **Contacts:** manually add/edit all emails and phones, choose recipients, exclude individual addresses from campaigns, copy contacts between workspaces, automatically scan loosely structured spreadsheet tabs, and sync approved contacts.
...
- **Templates:** separate workspace-scoped editor, saved templates, subject, message, personalization fields, inline JPEG/PNG/WebP images, and downloadable PDF or PowerPoint (`.pptx`) attachments. Files upload in resumable chunks and the editor shows the combined size against Gmail's 25 MB limit.
- **Communication:** select a workspace template and recipients, create a frozen draft, preview it, then explicitly send. Each distinct address gets one MIME email with inline images and its own copies of the selected PDF/PPTX attachments. Actual Gmail IDs, timestamps, sender and per-recipient status are retained.
...
Every linked-spreadsheet scan uses Google's current [grid row and column counts](https://developers.google.com/workspace/sheets/api/reference/rest/v4/spreadsheets/sheets#GridProperties), with resumable pages of at most 200 rows or about 20,000 cells. There is no fixed row-10,000 or ZZ-column cutoff, including for older import requests. A new scan refreshes the dimensions, so later rows and columns are included. Source data is read before corrections are written to `Gather Contacts`. Names are matched to existing contacts by a unique Gather ID, otherwise a unique normalized email match. Conflicting matches keep their email contacts with unresolved names blank. Clear source columns take precedence over old managed-tab guesses; email domains never supply business names. Existing contact IDs, exclusions, phones, unrelated notes, and frozen campaign recipients are preserved. Corrections synchronize across workspaces linked to that sheet in the same account.

Only unclear names use AI, in batches of up to 10 contacts with eligible nearby cells. This uses the account's saved encrypted provider key and selected Claude, Gemini, or compatible model, independently of the card-extraction switch. Responses must cite exact source cells and text fragments; unsupported names are rejected and AI-provided emails are ignored. Metadata, IDs, role and notes columns cannot supply names. Transient provider failures receive one retry, honoring provider retry times. Valid email contacts are saved before AI processing; missing credentials, invalid responses, or provider failures leave name resolution pending with an actionable error. Saving updated model settings resumes pending work. A new scan rereads source edits.

Import job IDs are generated on the server and scoped to the signed-in account. The automatic `/api/workspaces/:id/import-sheet` response includes `jobId`, `phase`, `status`, `imported`, `repaired`, `unresolved`, `pending`, `retryAt`, and `needsContinuation`. Subsequent requests carry the returned job ID; `restart: true` explicitly begins a new scan. Parser version, source coordinates, repair history, and cursors survive restarts in account storage. Contact repairs and sync writes run in groups of up to 200. Previously linked sheets automatically receive the new repair pass when Gather opens. Keep the app open while progress runs; unfinished jobs resume on return. Work pauses while a form dialog, Settings, or the template editor is open so it does not disrupt editing. There is no independent background worker.
